...
class detector(Node):
    ...
    def objectDetection_(self, frame):
        results = self.model_.predict(frame, device = 0, conf = 0.5, verbose = False)
        centerPts = []
    ...
    def trackingSystem_(self, detection):
        ...
        for tracker_id, xy in detection.items():
            ...
            if tracker_id not in self.activeTracking_:
                self.activeTracking_[tracker_id] = [[[xy[0], xy[1]]], 0]
            else:
    ...
    def cameraCallback_(self, msg):
        try:
            ...
            for tracking_id, targetLoc in self.activeTracking_.items():
                if len(targetLoc[0]) > 1:
                    predX, predY = self.polyPredict(3, targetLoc[0])
                    filter_ = KalmanFilter(0.0001, 2, 0, 1, 0.1, 0)
                    x, y = filter_.predict()
                    x1, y1 = filter_.update([predX[-1], predY[-1]])
                    if self.is_within_roi([y1[0, 1], x1[0, 0]], self.ROI) and targetLoc[1] < 4:
                        self.shoot(y1[0, 1], x1[0, 0])
                        targetLoc[1] += 1
